...
`setup.sh` optionally triggers a full Ansible run. If you chose to run it later:

```bash
# Deploy the monitoring stack to the monitoring server
# (installs Podman, loads images, pushes configs, starts the stack)
ansible-playbook ansible/playbooks/deploy-monitoring-server.yml
...
### Phase 3 — Ansible controller setup (air-gapped)

```bash
# Run the setup wizard — it installs Ansible collections from offline tarballs,
# generates all config files, then optionally runs the full Ansible deployment
bash setup.sh
```

Ansible handles everything from here: installs Podman and podman-compose on the
monitoring server, loads container images, pushes configs, and starts the stack.
`scripts/load-images.sh` is available as a manual fallback only.
...
|------|---------|-------------|
| `docker-compose.yml` | Container orchestration (podman-compose) | `setup.sh` (or edit manually) |
...
Edit `docker-compose.yml` (re-deploy via `ansible-playbook ansible/playbooks/deploy-monitoring-server.yml`) to change retention:
...
```bash
# Re-run the monitoring server playbook to push updated config and reload
ansible-playbook ansible/playbooks/deploy-monitoring-server.yml
...
```bash
# Run these on the monitoring server inside /opt/observability
...
# Validate Prometheus config without restarting
podman run --rm -v "$(pwd)/config/prometheus:/etc/prometheus:ro" \
...
# Check Podman image and volume usage
podman system df
du -sh /opt/observability/

# Remove unused images and stopped containers
podman system prune -f

# Reduce Prometheus retention — edit docker-compose.yml on the controller,
# then redeploy: ansible-playbook ansible/playbooks/deploy-monitoring-server.yml
