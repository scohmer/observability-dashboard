#!/usr/bin/env bash
# load-images.sh
# Loads Docker images from offline-packages/docker-images/ into the local
# Docker daemon on the air-gapped monitoring server.
#
# Usage:
...
echo
echo -e "${CYAN}═══════════════════════════════════════════════════════════${NC}"
echo -e "${CYAN}  Docker Image Loader — Air-Gapped Deployment${NC}"
echo -e "${CYAN}═══════════════════════════════════════════════════════════${NC}"
echo
...
for image_file in "${IMAGE_FILES[@]}"; do
  ...
  # Load the image
  if docker load < "${image_file}" 2>&1; then
...
if [[ "$DRY_RUN" == "false" && $LOADED -gt 0 ]]; then
  echo
  info "Loaded images:"
  docker images --format "  {{.Repository}}:{{.Tag}}\t{{.Size}}\t{{.CreatedAt}}" | grep -E "grafana|prometheus|loki|promtail" | sort || true
  echo
fi
...
if [[ "$DRY_RUN" == "false" ]]; then
  ...
  echo
  echo "Next steps:"
  echo "  1. Run setup.sh:           ./setup.sh"
  echo "  2. Start the stack:        docker compose up -d"
  echo "  3. Check stack status:     docker compose ps"
